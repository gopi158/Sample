...
class MailboxModule extends CWebModule
{
	 ...
	 */
	const TBL_CONV = 'pv_mailbox_conversation';
	const TBL_MSG = 'pv_mailbox_message';
	public $userid;
	/**
	...
	*/
	...
	public function getUserId($username='')
	{
		...
		{
			if(!$this->_userid)
				$this->_userid = Yii::app()->session['login']['id'];
				// Yii::app()->user->{$this->userIdColumn};
			return $this->_userid;
		}
	}
	...
	public function getUserName($userid=0)
	{
		if($userid)
		{
			//echo $userid;
			//exit;
			$r = call_user_func(array($this->userClass, 'model'))->findByAttributes(array('userid'=>$userid));
			//print_r($r->{$this->usernameColumn});
			//exit;
	...
	public function isAdmin($userid=0)
	{
		...
		return call_user_func(array($this->userClass, 'model'))->findByPk($userid)->{'username'};
	...
	*/
	public function autoComplete($term)
	{
		...
		$criteria->condition .= "t.email != '".$this->usernameColumn."'";
		...
		$sql = "select ".$select." from 
					(
					select pvusr.* from pv_users as pvusr join pv_invite_friend as pvf  WHERE CASE pvf.source
				      WHEN 'email' THEN pvusr.email = pvf.invitee_social_network_id and pvf.status = 1
				      WHEN 'facebook' THEN pvusr.socialnetworkid = pvf.invitee_social_network_id and pvf.status = 1
				    END AND pvf  .invited_by_user_id = ".$userid ." 
					union
					 select pur.* from pv_users pur 
         join 
     (select * from pv_invite_friend where CASE source WHEN 'email' THEN invitee_social_network_id in
     (select email from pv_users where userid = ".$userid .") WHEN 'facebook' THEN invitee_social_network_id in (select socialnetworkid from pv_users where userid = ".$userid  .") END) as
     pvif on pur.userid = pvif.invited_by_user_id and pvif.status = 1 where 1=1  
					) pv order by fname; ";
		...
		$criteria = new CDbCriteria;
		$criteria->condition = "t.userid in (".$userids.")";
	...
	public function beforeControllerAction($controller, $action)
	{
		...
		if (!isset(Yii::app()->session['login'])) {
			if($controller->getId()!='news')
			{
				...
				//exit;
				$controller->redirect($controller->createUrl('/site/index'));
