...
<div class="update-finao-box" id="hideimgvideo" style="width:225px!important;border:none!important;">
                	<div class="update-finao-area">
                    	
                    </div>
                    
                   <?php /*?>  <div class="upload-finao-media" style="width:225px;">
                    ...
                    <div class="uploaded-media" style="width:230px!important; padding-left:0; background:none;">
                    	 <!--newfinaovideo.php-->
...
<?php /*?><div id="closefinaodiv" >
	...
	<div class="">
		...
		<div class="padding-10pixels" style="margin-top:10px;">
		...
		<td width="2%">
			...
			<input type="radio" name="rduploadvideo" value="videoembedurl" onclick="js:hidedivs('rightjournal','video_embcode')"/>  </td><td class="orange font-15px" width="65%">
			  Youtube   
		</td>	
		</tr>	
		</table>
	  ...
	  <!--<div class="padding-10pixels" style="margin-top:10px;">
        <select onchange="show(this.value);" class="dropdown-grid" style="width:230px;">
        <option value="video">Upload Video</option> 
        <option value="utube">Paste Youtube URL</option>
        </select>
        </div>-->
            <div id="rightjournal">
<div class="error-message" id="video-alert" style="display:none;">Video will publish only after encoded.</div>
			...
			<span style=" position:absolute; top:0px;left:0px; margin-bottom:10px; ">
			<textarea style="width:222px!important; height:52px; resize:none;" class="finaos-area" name="description" id="video" placeholder="Enter Text" ></textarea></span>
            <div class="padding-10pixels left"><input type="file" id="file" name="file" class="upload-btn video_val"  onchange="readURLs(this.value);" onfocus="resetfile('errormsgv','')" style="width:190px;" /></div>
  <div class="clear"></div>
			<div class="padding-10pixels"><input type="text" name="title" id="description" value="" placeholder="Enter Caption" onclick="if(placeholder == 'Enter Caption') placeholder = ''" onblur="if(!placeholder) placeholder='Enter Caption'" class="txtbox" style="width:222px;" />	</div>				  

			<input type="hidden" id="hdnimgchange" value="" />

				
				<input type="submit" value="Add Video" id="sb" name="sb" class="orange-button" onclick="if($('#hdnimgchange').val() != '' ){sbclick();}else { if(validatesubmit('hdnimgchange','errormsgv','Please select a file to Upload','','') == '1') {return false;}} "/>
                
                <input type="button" onclick="$('#videoform').html('');$('#videoview').show(); " value="Cancel" class="orange-button" />
			...
			<div id="video_embcode" class="upload-image" style="display:none; padding-top:0; padding-bottom:0; padding-left:0; background:none;">
			...
			<div class="padding-10pixels left">
               ...
               <span style=" position:absolute; top:0px;left:0px; margin-bottom:10px;"> <textarea style="width:222px!important; height:52px; resize:none;" class="finaos-area" name="description" id="getjournalmsg" placeholder="Enter Text"><?php if(!empty($journalmsg)){ echo $journalmsg;} ?></textarea></span>
                <div class="padding-10pixels"><input type="text" name="txtVidembedUrl" id="txtVidembedUrl" class="txtbox" style="width:222px;" value="Add Video URL" onclick="if(value == 'Add Video URL') value = ''; if($('#txtVidembedUrl').hasClass('txtbox-caption-error')) $('#txtVidembedUrl').addClass('txtbox-caption').removeClass('txtbox-caption-error'); $('#errormsg').html('');" onblur="if(this.value == ''){ this.value='Add Video URL'; }else { allowonlyYoutube(this.id,'errormsg','videopage'); }  " /></div>
				
				<div class="padding-10pixels"><input type="text" name="vdurldescription" id="vdurldescription" value=""  placeholder="Enter Caption" onclick="if(placeholder == 'Add Caption') placeholder = ''; " onblur="if(this.placeholder == '') { this.placeholder='Add Caption'; } " class="txtbox" style="width:222px" /></div>					  

				<input   type="button" value="Add Video" style="margin-right:3px;" id="sburl" name="sburl" class="orange-button left" style="cursor:pointer;" onclick="if($('#txtVidembedUrl').val() != 'Add Video URL' ){  saveadminvideoembUrl(<?php echo $userid ?>,'<?php echo $groupid; ?>','','group','','txtVidembedUrl','vdurldescription','videopage','errormsg'); }else{ if(validatesubmit('txtVidembedUrl','errormsg','Please enter an url to Save','txtVidembedUrl','Add Video URL') == '1') {return false;} };"/>
...
function hidedivs(hideid,showid)
{
	...
	if(showid == 'video_embcode')
	{
		//alert("utube");
		/*var msg  = $('#video').val();*/
		var msg = document.getElementById('video').value;
		//alert(msg);
		$("#getjournalmsg").val(msg);
		 
	}
	else
	{
		//alert("video");
		/*var msg  = $('#getjournalmsg').val();*/
		var  msg =  document.getElementById('getjournalmsg').value;
		//alert(msg);
		$("#video").val(msg);
...
function saveadminvideoembUrl(userid,groupid,finaoid,sourcetype,journalid,txtid,txtdescid,sourcpage,errorid)
{
	...
	var url='<?php echo Yii::app()->createUrl("/finao/GetadminVideodetail"); ?>';
	$.post(url, {userid:userid, finaoid : finaoid, upload : upload , sourcetype:sourcetype, journalid: journalid , emburl: emburl, embdescr:embdescr, tileid : tileid , tilename:tilename,journaltxt:journaltxt,groupid:groupid,ismember:ismember},
		function(data){
			...
			if(data)
			{
				 var rd ='<?php echo Yii::app()->createUrl("/group/dashboard/groupid/##data1"); ?>';
	                    			rd = rd.replace('##data1',data);
