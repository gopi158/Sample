...
<?php /*?><div id="closefinaodiv" >
	...
	<div class="">
		...
		<td width="2%">
			...
			Upload <?php echo strtoupper($sourcetype); ?> Video
		</td><td width="2%">
			...
			Paste Youtube URL to embed video for <?php echo strtoupper($sourcetype); ?>
		</td>	
		</tr>	
		</table>
...
<div class="error-message" id="video-alert" style="display:none;">Video will be published only after it is encoded.</div>
			...
			<textarea class="update-finao-textarea" name="description" id="getjournalmsg" placeholder="Update this FINAO"><?php if(!empty($journalmsg)){ echo $journalmsg;} ?></textarea></span>
            <input type="file" id="file" name="file" class="upload-btn video_val"  onchange="readURLs(this.value);" onfocus="resetfile('errormsgv','')" style="width:190px;" />				   

				<input type="text" name="title" id="description" value="" placeholder="Enter Caption" onclick="if(placeholder == 'Enter Caption') placeholder = ''" onblur="if(!placeholder) placeholder='Enter Caption'" class="txtbox left" style="width:160px" maxlength="60" />					  

			<input type="hidden" id="hdnimgchange" value="" />

				
				<input type="submit" value="Update FINAO" id="sb" name="sb" class="orange-button" onclick="if($('#hdnimgchange').val() != '' ){sbclick();}else { if(validatesubmit('hdnimgchange','errormsgv','Please select a file to Upload','','') == '1') {return false;}} "/>
			...
			<div class="padding-10pixels left">
               ...
               <span style=" position:absolute; top:0px;left:0px; "> <textarea class="update-finao-textarea" name="description" id="getjournalmsg" placeholder="Add Journals to your FINAO"><?php if(!empty($journalmsg)){ echo $journalmsg;} ?></textarea></span>
				...
				<input   type="button" value="Update FINAO" style="margin-right:3px;" id="sburl" name="sburl" class="orange-button left" style="cursor:pointer;" onclick="if($('#txtVidembedUrl').val() != 'Add Video URL' ){  savevideoembUrl(<?php echo $_POST['userid'] ?>,'<?php echo $groupid; ?>',<?php echo $finaoinfo->user_finao_id; ?>,'<?php echo $sourcetype; ?>',<?php echo $journalid; ?>,'txtVidembedUrl','vdurldescription','videopage','errormsg'); }else{ if(validatesubmit('txtVidembedUrl','errormsg','Please enter an url to Save','txtVidembedUrl','Add Video URL') == '1') {return false;} };"/>
