...
// image view size
static const CGFloat CSToastImageViewWidth      = 20;
static const CGFloat CSToastImageViewHeight     = 15;
...
static const NSString * CSToastActivityKey  = @"CSToastActivityKey";
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position  WithDelegate:(UIViewController *)delegate{
    UIView *toast = [self viewForMessage:message title:nil image:[UIImage imageNamed:@"logo_finao"]];
    [self showToast:toast duration:duration position:position WithDelegate:(UIViewController *)delegate];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title {
    UIView *toast = [self viewForMessage:message title:title image:[UIImage imageNamed:@"logo_finao"]];
    [self showToast:toast duration:duration position:position];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title WithDelegate:(UIViewController *)delegate{
    UIView *toast = [self viewForMessage:message title:title image:[UIImage imageNamed:@"logo_finao"]];
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point WithDelegate:(UIViewController *)delegate{
    // associate the activity view with self
    objc_setAssociatedObject (toast,
                               &CSToastActivityKey,
                               delegate,
                               OBJC_ASSOCIATION_RETAIN_NONATOMIC);
     [self showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point];
}
...
- (void)handleToastTapped:(UITapGestureRecognizer *)recognizer {
    ...
    HomeScreenViewController *existingActivityController = (HomeScreenViewController *)objc_getAssociatedObject(self, &CSToastActivityKey);
    if(existingActivityController == nil)
        return;
    [existingActivityController LaunchActivity];
   // if([existingActivityController respondsToSelector:@selector(LaunchActivity:)]){
    //    [existingActivityController performSelector:@selector(LaunchActivity:) withObject:nil afterDelay:0.1];
    //}
}
...
- (void)makeToastActivity:(id)position {
    ...
    activityView.backgroundColor = [[UIColor orangeColor] colorWithAlphaComponent:CSToastOpacity];
    ...
    [activityView addGestureRecognizer:singleFingerTap];
    
    if (CSToastDisplayShadow) {
        activityView.layer.shadowColor = [UIColor orangeColor].CGColor;
...
- (UIViewController *)FindViewController {
    UIResponder *responder = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
...
//The click on toast goes to Notifications list
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = [UIColor orangeColor].CGColor;
        wrapperView.layer.shadowOpacity = CSToastShadowOpacity;
        wrapperView.layer.shadowRadius = CSToastShadowRadius;
        wrapperView.layer.shadowOffset = CSToastShadowOffset;
    }
    
    wrapperView.backgroundColor = [[UIColor orangeColor] colorWithAlphaComponent:CSToastOpacity];
